...
_interpolator_image = 'nearest'          # interpolator image
_interpolator_label = 'nearest'          # interpolator label

Segmentation = (opt.segmentation == 1)
debug = False
...
def create_list(data_path):
    ...
    list_source = [{'data': os.path.join(path, data_name)}
                   for path in data_list]
    list_target = [{'label': os.path.join(
        path, label_name)} for path in data_list]
...
def resample_sitk_image(sitk_image, spacing=None, interpolator=None, fill_value=0):
    ...
    print('sitk_interpolator: ', sitk_interpolator)
    print('new_size: ', new_size)
    print("Total size:",   orig_size * orig_spacing)
    print('new_size: ', new_size)
    print('--------------------++++++++++++++++>num_dim: ', num_dim)
    print('--------------------++++++++++++++++>num_size: ', num_size)

    resampled_sitk_image = sitk.Resample(sitk_image, new_size,
                                         sitk.Transform(), sitk_interpolator, orig_origin,
                                         new_spacing, orig_direction, fill_value,
                                         orig_pixelid)
...
def rotation3d_image(image, theta_x, theta_y, theta_z):
    ...
    euler_transform = sitk.Euler3DTransform(
        get_center(image), theta_x, theta_y, theta_z, (0, 0, 0))
...
def translateit(image, offset, isseg=False):
    ...
    array = scipy.ndimage.interpolation.shift(
        array, (int(offset[0]), int(offset[1]), 0), order=order)
...
class NiftiPromptDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_path,
                 ...
                 train=True,
                 test=False,
                 add_graph=False,label2mask = False):

        # Init membership variables
        self.data_path = data_path
        upper_index = 50 if debug is False else 10 
        # self.images_list = glob(os.path.join(data_path,path_A , '*.nii.gz'), recursive=False)
        self.images_list = lstFiles(os.path.join(data_path, path_A))
        print('os.path.join(data_path, path_A): ', os.path.join(data_path, path_A))
        print("-----> orginal length of the dataset:", len(self.images_list))
        self.images_list = self.images_list[:upper_index]   
        print("-----> current length of the dataset being used!!!:", len(self.images_list))
        self.labels_list = None
        print('self.images_list : ', self.images_list )
        
        
        if Segmentation:
            self.labels_list = lstFiles(os.path.join(data_path, path_B))[:upper_index]
        else:
            self.labels_list = lstFiles(os.path.join(data_path, 'super'))[:upper_index]
        # self.images_list = [ '/media/xiaodan/3d_synthesis/Train/img/AIIB23_74.nii.gz']
        # self.labels_list = [ '/home/xiaodan/PycharmProjects/3Dsyngouthesis/travel/test.nii']
        print('labels_list: ',  self.labels_list)
        ...
        self.prompts = pandas.read_csv(
        "../../data/3D-CycleGan-Pytorch_data/data_info_all_prompts.csv")
        print('self.prompts: ', len(self.prompts))
    ...
    def label2masks(self, masked_img):
        map = { "LUNG1": -0.500, "LUNG2" :0.000, "AIRWAY" :0.500}
        result_img = np.zeros(masked_img.shape + (len(map),), dtype=np.float32)
        for i, label in enumerate(map):
            result_img[masked_img == map[label], i] = 1

        result_img = np.squeeze(np.transpose(result_img, (0,4,1,2,3)), axis=0)

        return result_img

  

    def preprocess_onehot(self, label_map):
        label_map = torch.LongTensor(label_map).unsqueeze(0)
        bc, _, h, w, z = label_map.size()
        nc = 4
        input_label = torch.FloatTensor(bc, nc, h, w, z).zero_()
        onehot_semantics = input_label.scatter_(1, label_map, 1.0)

        return onehot_semantics[0]



    def get_one_instance(self,data_path, label_path, file_name):
        ...
        # (width, height, depth))
        print('size = image.GetSize(): ', image.GetSize())
        # (width, height, depth))
        print('size = label.GetSize(): ', label.GetSize())

        # 重新采样是,减少加载是时间
        # image, label = self.resample_image(image, label, new_spacing=[1.0, 1.0, 4.0])
        ...
        if self.add_graph:
            graph = self.read_image(data_path.replace('img', 'awlung_sketch'))
            castImageFilter.SetOutputPixelType(self.bit)
            graph = castImageFilter.Execute(graph)
            sample = {'image': image, 'label': [label, graph]}
        else:
            sample = {'image': image, 'label': label}

        # apply the transforms to image and label (normalization, resampling, patches)
        # (width, height, depth))
        print('size = image.GetSize(): ', image.GetSize())
        # (width, height, depth))
        print('size = label.GetSize(): ', label.GetSize())
        ...
        print('convert sample np array---------------------->: ')
        image_np = sitk.GetArrayFromImage(sample['image'])

        print(
            "finished sitk.GetArrayFromImage(sample['image'])---------------------->: ")
        image_np = np.transpose(image_np, (2, 1, 0))
        print("finished np.transpose(image_np, (2, 1, 0))---------------------->: ")
        image_np -= 127.5
        image_np /= 127.5
        print("finished (image_np - 127.5) / 127.5---------------------->: ")
        image_np = image_np[np.newaxis, :, :, :]

        if self.add_graph:
            label_np = abs(sitk.GetArrayFromImage(sample['label'][0]))
            label_np = np.transpose(label_np, (2, 1, 0))
            graph_np = abs(sitk.GetArrayFromImage(sample['label'][1]))
            graph_np = np.transpose(graph_np, (2, 1, 0))

            # the graph is more accurate, so use graph as instance label
            graph_np = self.preprocess_onehot(graph_np[np.newaxis, :, :, :])
            label_np = label_np[np.newaxis, :, :, :]
            label_np -= 2
            label_np /= 2

            label_np = np.concatenate([label_np, graph_np])

        else:
            print('convert sample to tf tensors---------------------->: ')
            label_np = sitk.GetArrayFromImage(sample['label'])
            print("+++++++++++++++label_np.shape", label_np.shape)
            print("++++++++++++++++image_np.shape", image_np.shape)

            print(
                "finished sitk.GetArrayFromImage(sample['label'])---------------------->: ")
            label_np = np.transpose(label_np, (2, 1, 0))
            print("finished np.transpose(label_np, (2, 1, 0))---------------------->: ")
            ...
            label_np = label_np[np.newaxis, :, :, :]
            print(
                "finished label_np[np.newaxis, :, :, :]---------------------->: ")

        print('finsihed_loader process---------------------->: ')

        prompt = self.prompts[self.prompts['File Name']
                              == file_name]['Prompt'].iloc[0]
        print('prompt: ', prompt)

        image_id = data_path.split("/")[-1].replace(".nii.gz", "")
        # this is the final output to feed the network

        if self.label2mask:
            label_mask = self.label2masks(label_np)
            return {"image": torch.from_numpy(image_np), "labelmask": torch.from_numpy(label_mask), "label": torch.from_numpy(label_np), "prompt": prompt, "image_id": image_id}

        return {"image": torch.from_numpy(image_np), "label": torch.from_numpy(label_np), "prompt": prompt, "image_id": image_id}
    ...
    def __getitem__(self, index):
        ...
        assert file_name == label_path.split("/")[-1]
        ...
        if self.test is False:
            result = self.get_one_instance(data_path=data_path,
                              label_path=label_path, file_name=file_name)
        else:
            result = {"label_path": label_path, "data_path":data_path, "file_name":file_name} 
...
def preprocessing(sitk_image, new_spacing, crop_size, spacing=True):
    ...
    resampled_sitk_image = sitk.Resample(sitk_image, new_size,
                                         sitk.Transform(), sitk.sitkNearestNeighbor, sitk_image.GetOrigin(),
                                         new_spacing, sitk_image.GetDirection(), 0,
                                         sitk_image.GetPixelIDValue())
    if crop_size is not None:
        roiFilter = sitk.RegionOfInterestImageFilter()
        roiFilter.SetSize([crop_size[0], crop_size[1], crop_size[2]])

        if new_size[0] <= crop_size[0]:
            start_i = 0
        else:
            start_i = int((new_size[0] - crop_size[0])/2)

        if new_size[1] <= crop_size[1]:
            start_j = 0
        else:
            start_j = int((new_size[1] - crop_size[1])/2)

        start_k = new_size[2] - crop_size[2]
        # start_i, start_j, start_k = 40,40,40
        roiFilter.SetIndex([start_i, start_j, start_k])

        image_crop = roiFilter.Execute(resampled_sitk_image)
    else:
        image_crop = resampled_sitk_image
    return image_crop


class NifitDataSet_label(torch.utils.data.Dataset):
    ...
    def __init__(self, data_path,
        ...
        # Init membership variables
        self.data_path = data_path
        self.images_list = lstFiles(os.path.join(data_path, path_A))[:50]
        if Segmentation:
            self.labels_list = lstFiles(os.path.join(data_path, path_B))[:50]
        else:
            self.labels_list = lstFiles(os.path.join(data_path, 'super'))[:50]
        # self.images_list = [ '/media/xiaodan/3d_synthesis/Train/img/AIIB23_74.nii.gz']
        # self.labels_list = [ '/home/xiaodan/PycharmProjects/3Dsynthesis/travel/test.nii']
    ...
    def __getitem__(self, index):
        ...
        label = self.read_image(label_path)  # set intensity 0-255
        castImageFilter.SetOutputPixelType(self.bit)
        label = castImageFilter.Execute(label)
...
def Normalization(image):
    ...
    intensityFilter.SetWindowMaximum(150)
    intensityFilter.SetWindowMinimum(-1000)
...
class Registration(object):
    ...
    def __call__(self, sample):
        image, image_sobel, label, label_sobel,  = sample[
            'image'], sample['image'], sample['label'], sample['label']
...
class Resample(object):
    ...
    def __call__(self, sample):
        ...
        if check is True:
            print("Resampling is currently doing!")
            if isinstance(label, list):
                print("Resampling is a list")
                label_new = []
                for label_inst in label:
                    label_inst.SetSpacing(image.GetSpacing())
                    label_new.append(resample_sitk_image(label_inst, spacing=new_resolution,
                                                         interpolator=_interpolator_label))
                image = resample_sitk_image(
                    image, spacing=new_resolution, interpolator=_interpolator_image)

                return {'image': image, 'label': label_new}
            else:
                print("Resampling is not a list")
                print("Image currently!")
                image = resample_sitk_image(
                    image, spacing=new_resolution, interpolator=_interpolator_image)
...
class CropBackground(object):
    ...
    def __call__(self, sample):
        ...
        roiFilter.SetIndex([int(x_centroid-(size_new[0])/2),
                           int(y_centroid-(size_new[1])/2), 0])
...
class CenterCrop(object):
    """
    Crop the center of images in a sample.
    ...
    def __init__(self, output_size, drop_ratio=0.1, min_pixel=1):
        ...
        if min_pixel >= 0:
            ...
            raise RuntimeError(
                'Min label pixel count should be integer larger than 0')
    ...
    def __call__(self, sample):
        ...
        if size_old[0] <= size_new[0]:
            ...
            start_i = int((size_old[0] - size_new[0])/2)

        if size_old[1] <= size_new[1]:
            start_j = 0
        else:
            start_j = int((size_old[1] - size_new[1])/2)

        if size_old[2] <= size_new[2]:
            start_k = 0
        else:
            start_k = int((size_old[2] - size_new[2])/2)
        ...
        if Segmentation is True:
            if isinstance(label, list):
                ...
                label_inst_crop = roiFilter.Execute(label[0])
                statFilter = sitk.StatisticsImageFilter()
                statFilter.Execute(label_inst_crop)
                label_new.append(label_inst_crop)
                for label_inst in label[1:]:
                    label_inst_crop = roiFilter.Execute(label_inst)
                    label_new.append(label_inst_crop)
                label_crop = label_new
            else:
...
#         image, label = sample['image'], sample['label']
#         size_old = image.GetSize()
#         print('size_old of image: ', size_old)
#         print('size_old of label: ', label.GetSize())
#         size_new = self.output_size
...
#             # start_i, start_j, start_k = 40,40,40
#             roiFilter.SetIndex([start_i, start_j, start_k])
#             print('[start_i, start_j, start_k]: ', [start_i, start_j, start_k])


#             if Segmentation is False:
#                 print("Segmentation is False")
...
#             print('statFilter.GetSum(): ', statFilter.GetSum())
...
#             else:
#                 contain_label = True
#             print("Current in RandomCrop operations!!")
...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        ...
        while not contain_label:
            ...
            # 应对的相同的分辨率的label 和 image 的大小尺寸
            # Randomly choose to sample from the lower or upper interval
            start_i = np.random.randint(
                0, size_old[0] - size_new[0] + 1) if size_old[0] > size_new[0] else 0
            start_j = np.random.randint(
                0, size_old[1] - size_new[1] + 1) if size_old[1] > size_new[1] else 0
            start_k = np.random.randint(
                0, size_old[2] - size_new[2] + 1) if size_old[2] > size_new[2] else 0

            # 应对的不同的分辨率的label 和 image 的大小尺寸
            # Calculate the minimum size considering both image and label
            # min_size = [min(size_old[i], size_old_label[i]) for i in range(3)]

            # # Calculate the starting indices for the ROI considering the minimum size
            # start_i = np.random.randint(0, min_size[0] - size_new[0] + 1) if min_size[0] > size_new[0] else 0
            # start_j = np.random.randint(0, min_size[1] - size_new[1] + 1) if min_size[1] > size_new[1] else 0
            # start_k = np.random.randint(0, min_size[2] - size_new[2] + 1) if min_size[2] > size_new[2] else 0
            ...
            if Segmentation is True:
                ...
                if isinstance(label, list):
                    ...
                    print("isinstance is False")
                    print('roiFilter.GetSize(): ', roiFilter.GetSize())
                    print("Size of the original label:", label.GetSize())
                    label_crop = roiFilter.Execute(label)
                    statFilter = sitk.StatisticsImageFilter()
                    statFilter.Execute(label_crop)
            ...
            print('statFilter.GetSum(): ', statFilter.GetSum())
            if statFilter.GetSum() < self.min_pixel:
                # has some probabilty to contain patch with empty label
                contain_label = self.drop(self.drop_ratio)
            else:
                contain_label = True
            print('contain_label: ', contain_label)
            print("Current in RandomCrop operations!!")

        image_crop = roiFilter.Execute(image)
        print("-----------------------------finshed random cropping!")
        return {'image': image_crop, 'label': label_crop}

    def drop(self, probability):
        number = random.random()
        print("current prob:---> ", number)
        print('number <= probability: ', number <= probability)
...
class Augmentation(object):
    ...
    def __call__(self, sample):
        ...
        if choice == 2:  # Recursive Gaussian
            ...
            image = self.noiseFilter.Execute(image)
            if Segmentation is False:
                label = self.noiseFilter.Execute(
                    label)    # comment for segmentation
        ...
        if choice == 3:  # Random rotation
            ...
            if isinstance(label, list):
                label_new = []
                for label_inst in label:
                    label_new.append(rotation3d_label(
                        label_inst, theta_x, theta_y, theta_z))
                return {'image': image, 'label': label_new}
            else:
        ...
        if choice == 4:  # BSpline Deformation
            ...
            bspline.SetTransformDomainPhysicalDimensions(
                domain_physical_dimensions)
            bspline.SetTransformDomainMeshSize((10, 10, 10))

            # Random displacement of the control points.
            originalControlPointDisplacements = np.random.random(
                len(bspline.GetParameters())) * self.randomness
            ...
            if isinstance(label, list):
                label_new = []
                for label_inst in label:
                    label_new.append(sitk.Resample(label_inst, bspline))
                return {'image': image, 'label': label_new}
            else:
        ...
        if choice == 5:  # Random flip
            ...
            if isinstance(label, list):
                label_new = []
                for label_inst in label:
                    label_new.append(flipit(label_inst, axes))
                return {'image': image, 'label': label_new}
            else:
        ...
        if choice == 8:  # translate
            ...
            if isinstance(label, list):
                label_new = []
                for label_inst in label:
                    label_new.append(translateit(label_inst, offset))
                return {'image': image, 'label': label_new}
            else:
...
class ConfidenceCrop(object):
    ...
    def __call__(self, sample):
        ...
        if labelShapeFilter.GetNumberOfLabels() == 0:
            ...
            selectedLabel = 0
            centroid = (int(
                self.output_size[0] / 2), int(self.output_size[1] / 2), int(self.output_size[2] / 2))
        else:
            # randomly select of the label's centroid
            selectedLabel = random.randint(
                1, labelShapeFilter.GetNumberOfLabels())
            centroid = label.TransformPhysicalPointToIndex(
                labelShapeFilter.GetCentroid(selectedLabel))
        ...
        for i in range(3):
            ...
            if centroid[i] < (self.output_size[i] / 2):
                ...
                centroid[i] = image.GetSize()[i] - \
                    int(self.output_size[i] / 2) - 1

            # get start point
            while ((start[i] < 0) or (end[i] > (image.GetSize()[i] - 1))):
                offset[i] = self.NormalOffset(
                    self.output_size[i], self.sigma[i])
                start[i] = centroid[i] + offset[i] - \
                    int(self.output_size[i] / 2)
    ...
    def NormalOffset(self, size, sigma):
        # 100 sample is good enough
        s = np.random.normal(0, size * sigma / 2, 100)
