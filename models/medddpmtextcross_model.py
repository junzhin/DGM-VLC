...
class medddpmtextcrossmodel(BaseModel):
    def name(self):
        return "medddpmtextcrossmodel"
    ...
    def initialize(self, opt):
        ...
        self.backbone = create_model(self.opt.patch_size[0], self.opt.num_channels, self.opt.num_res_blocks, in_channels=self.opt.input_nc, out_channels=self.opt.output_nc,model_type = "prompt1", opt = self.opt).to(self.opt.device)
    ...
    def forward(self):
        ...
        loss= self.netG_A(self.real_B, condition_tensors=self.real_A, text_embeddings = self.read_A_prompt_embeddings)
    ...
    def test(self):
        with torch.no_grad():
            ...
            all_images_list = list(map(lambda n: self.ema_model.sample(batch_size=n, condition_tensors=self.sample_input, text_embeddings = self.read_A_prompt_embeddings), batches))
