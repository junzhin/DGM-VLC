...
from models.model_helper import define_G, define_D
...
class pix2pixclipdfModel(BaseModel):
    def name(self):
        return "pix2pixclipdfModel"
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            self.optimizer_G = torch.optim.Adam(
                self.netG_A.parameters(), lr=opt.lr, betas=(0.0, 0.9)
            ) 
            ...
            self.optimizer_D = torch.optim.Adam(
                self.netD_A.parameters(), lr=opt.lr*4, betas=(0.0, 0.9)
            )
    ...
    def forward(self):
        ...
        print('self.read_A_prompt_embeddings: ',
              self.read_A_prompt_embeddings.device)
        print('self.real_A: ', self.real_A.device)
        self.fake_B = self.netG_A(self.real_A, prompts = self.read_A_prompt_embeddings, opt = self.opt)
