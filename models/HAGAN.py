...
class Sub_Encoder(nn.Module):
    def __init__(self, channel=256, latent_dim=1024):
        ...
        self.relu = nn.ReLU()
        self.conv2 = nn.Conv3d(channel // 4, channel // 4,
                               kernel_size=4, stride=2, padding=1)  # out:[16,16,16]
        self.bn2 = nn.GroupNorm(8, channel // 4)
        self.conv3 = nn.Conv3d(channel // 4, channel // 2,
                               kernel_size=4, stride=2, padding=1)  # out:[8,8,8]
        self.bn3 = nn.GroupNorm(8, channel // 2)
        self.conv4 = nn.Conv3d(
            channel // 2, channel, kernel_size=4, stride=2, padding=1)  # out:[4,4,4]
        self.bn4 = nn.GroupNorm(8, channel)
        self.conv5 = nn.Conv3d(
            channel, latent_dim, kernel_size=4, stride=1, padding=0)  # out:[1,1,1,1]
...
class Encoder(nn.Module):
    def __init__(self, channel=64):
        ...
        self.relu = nn.ReLU()
        # in:[16,128,128], out:[8,64,64]
        self.conv1 = nn.Conv3d(
            1, channel // 2, kernel_size=4, stride=2, padding=1)
        self.bn1 = nn.GroupNorm(8, channel // 2)
        self.conv2 = nn.Conv3d(channel // 2, channel // 2,
                               kernel_size=3, stride=1, padding=1)  # out:[8,64,64]
        self.bn2 = nn.GroupNorm(8, channel // 2)
        self.conv3 = nn.Conv3d(
            channel // 2, channel, kernel_size=4, stride=2, padding=1)  # out:[4,32,32]
...
class Sub_Discriminator(nn.Module):
    def __init__(self, num_class=0, channel=256):
        ...
        self.conv2 = SNConv3d(1, channel // 4, kernel_size=4,
                              stride=2, padding=1)  # out:[16,16,16]
        self.conv3 = SNConv3d(channel // 4, channel // 2,
                              kernel_size=4, stride=2, padding=1)  # out:[8,8,8]
        self.conv4 = SNConv3d(channel // 2, channel,
                              kernel_size=4, stride=2, padding=1)  # out:[4,4,4]
        self.conv5 = SNConv3d(
            channel, 1 + num_class, kernel_size=4, stride=1, padding=0)  # out:[1,1,1,1]
...
class Discriminator(nn.Module):
    def __init__(self, num_class=0, channel=512):
        ...
        # D^H
        self.conv2 = SNConv3d(1, channel // 16, kernel_size=4,
                              stride=2, padding=1)  # out:[8,64,64,64]
        self.conv3 = SNConv3d(channel // 16, channel // 8,
                              kernel_size=4, stride=2, padding=1)  # out:[4,32,32,32]
        ...
        self.conv7 = SNConv3d(
            channel, channel // 4, kernel_size=(1, 4, 4), stride=1, padding=0)  # out:[1,1,1,1]
    ...
    def forward(self, h, h_small, crop_idx):
        ...
        h = torch.cat(
            [h, (crop_idx / 112. * torch.ones((h.size(0), 1))).cuda()], 1)  # 128*7/8
    ...
    def forward(self, h, crop_idx=None, class_label=None):
        ...
        if crop_idx != None or self.mode == 'eval':
            ...
            if self.mode == "train":
                ...
                # Crop sub-volume, out: (4, 32, 32)
                h = h_latent[:, :, crop_idx // 4:crop_idx // 4 + 4, :, :]
            else:
                h = h_latent
